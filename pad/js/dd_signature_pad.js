...
var baseURL = "";
// Path to images
var dd_imgPath = "pad/images/dd_signature_pad";
...
// Save button name
var dd_objSaveButtonName = "Save Signature";
// Signature output to image file name
var dd_outputFileName = "sign";
// Data processor location file + path
var dd_dataProcessURL = "pad/dd_signature_process.php";
...
var dd_doPostAction = {
	dataSaved : function(f){
		...
		// Add your code here. The 'f' var is the filename passed back
			$(".signature-holder").html(`<img src="uploads/signatures/`+f+`" class="img-responsive">`);
			$("#dd_saveButton").text("Save Signature");
...
var dd_baseCanvas = {
	...
	'width' : 400,					// Canvas html width (not css width)
	'height' : 150,					// Canvas html height (not css width)
...
var dd_webkitCursorOffset = "";
// 'Draw' the signature area drawing to an existing element on page
...
var dd_buildStructure = function() {
    var wrapper = document.getElementById('dd_signaturePadWrapper');
    if (!wrapper) {
        alert('Error!!\nYou are missing the Div Element: \'dd_signaturePadWrapper\'');
        return
    }
    ...
    with(iconWrapper) {
        setAttribute('id', 'dd_signatureToolWrapper');
        with(style) {
            position = "relative";
    ...
    with(iconColorPicker) {
        ...
        setAttribute('title', 'Change Signature Color');
        addEventListener('click', function(event) {
    ...
    with(iconErasor) {
        ...
        setAttribute('title', 'Clear Signature Pad');
        addEventListener('click', function(event) {
        ...
        function mover(e) {
            ...
            if (touchEnabled) {
			    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
			        e._x = (e.changedTouches[0].pageX - 20) - offX;
			        e._y = (e.changedTouches[0].pageY - 240) - offY;
			    } else {
			        e._x = (e.changedTouches[0].pageX - 20) - offX;
			        e._y = (e.changedTouches[0].pageY - 120) - offY;
			    }
			    e.preventDefault()
            } else {
    ...
    var dd_signaturePad = new initCanvas(objCanvas)
};
